...
export default function ClinicRegistrationPage() {
  ...
  const handleClinicNext = () => {
    if (!clinicName.trim() || slug.length < 3 || !slugOk) return;
    setStep("account");
  };
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      // 1. Create Firebase Auth account for the manager
      const credential = await createUserWithEmailAndPassword(auth, email, password);
      const { user }   = credential;
      const displayName = `${firstName.trim()} ${lastName.trim()}`;
      await updateProfile(user, { displayName });

      // 2. Create the clinic document first (so we have the clinicId)
      ...
      // 4. Write physiotherapist profile doc (manager is also a physio record)
    ...
    <div style={{ minHeight: "100vh", background: "#f5f8fc", fontFamily: "'Outfit', sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        .cr-slug-prefix {
          ...
          font-size: 13px; color: #9a9590; pointer-events: none;
        }
        .cr-slug-input {
